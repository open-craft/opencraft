...
export function uiStateReducer(state = initialState,
    ...
    switch (action.type) {
        case Types.NAVIGATE_NEXT_PAGE:
            if (state.currentRegistrationStep == null)
                return initialState;
            const nextStep = Math.min(
                state.currentRegistrationStep + 1,
                RegistrationSteps.LAST_STEP,
            );
            return Object.assign({}, state, {currentRegistrationStep: nextStep});
        case Types.NAVIGATE_PREV_PAGE:
            ...
            return Object.assign({}, state, {currentRegistrationStep: prevStep});
        default:
            return state;
    }
}
