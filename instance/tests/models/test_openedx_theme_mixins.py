...
class OpenEdXThemeMixinTestCase(TestCase):
    ...
    def test_colors_and_images_applied(self):
        ...
        for variables in (ansible_theme_vars, ansible_vars):
            ...
            # Testing of files is a bit special because if we use SWIFT, we will see URLs, but in local tests we have
            # SWIFT disabled so it's ok to see file names. We allow for both.
